...
def login():
    if request.method == "POST":
        ...
        temp = cur.execute("select * from users where email=%s and password=%s",(email, password))
        result = cur.fetchone()
        cur.close()
        if temp != 0:
...
def register():
    if request.method == "POST":
        ...
        password = details['pass']
        cur = mysql.connection.cursor()
        cur.execute("insert into users(username, fullname, email, password) values(%s,%s,%s,%s)",(uname, fname, email, password))
...
def property(id):
    ...
    cur.close()
    col=['bedrooms','bathrooms','sqft_living','sqft_lot','floors','waterfront','view','condition','grade','sqft_above','sqft_basement','yr_built','yr_renovated','lat','long','sqft_living15','sqft_lot15'] 
    data = [[result[0][3], result[0][4], result[0][5], result[0][6], result[0][7], result[0][8], result[0][9], result[0][10], result[0][11], result[0][12], result[0][13], result[0][14], result[0][15], result[0][17], result[0][18], result[0][19], result[0][20]]]
...
def filter_data():
    ...
    query = "select * from house where zipcode in (select zipcode from area where area_name like '{}%' and price>={} and price<={} and sqft_living>={} and sqft_living<={})".format(data['searchkey'],data['minprice'],data['maxprice'],data['minarea'],data['maxarea'])
...
def update_profile():
    if request.method == "POST":
        ...
        email = details['email']
        cur = mysql.connection.cursor()
        cur.execute("update users set username='{}', fullname='{}', email='{}' where id='{}'".format(uname, fname, email,session['id']))
        mysql.connection.commit()
        cur.close()
...
def add_property():
    if request.method == "POST":
        ...
        id = uid + int(details['Sqft_living'])+ int(details['Sqft_living15'])+ int(details['Sqft_lot'])+ int(details['Sqft_lot15'])
        '''cur = mysql.connection.cursor()
        cur.execute("insert into house(id,date,price,bedrooms,bathrooms,sqft_living,sqft_lot,floors,waterfront,view,cond,grade,sqft_above,sqft_basement,yr_built,yr_renovated,zipcode,lat,lng,sqft_living15,sqft_lot15,added_by) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(id,today,details['Price'],details['bedroom '],details['bathroom'],details['Sqft_living'],details['Sqft_lot'],details['floors'],details['Waterfront'],details['view'],details['condition'],details['grade'],details['Sqft_above'],details['Sqft_basement'],details['Year_Built'],details['Year_Renovated'],details['Zipcode'],details['lat'],details['lng'],details['Sqft_living15'],details['Sqft_lot15'],uname))
        mysql.connection.commit()
        cur.close()'''
...
def my_properties():
    cur = mysql.connection.cursor()
    cur.execute("select * from house having added_by='{}'".format(session['uname']))
    result = cur.fetchall()
    cur.close()
